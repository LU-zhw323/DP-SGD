...
from aihwkit.simulator.configs.devices import ConstantStepDevice, PulsedDevice
...
class DPSimulatorTile(SimulatorTile, Module):
    ...
    def __init__(self, x_size: int, d_size: int, rpu_config: "DPRPUConfig", bias: bool = False):
    ...
    def update(
        ...
        if self._update.noise_multiplier:
            delta_w = (1/self._update.batch_size) * (torch.normal(mean=0, std=self._update.noise_multiplier * self._update.max_grad_norm, size=delta_w.size(), device=delta_w.device) * randn_like(delta_w))

        self._analog_weight = self._analog_weight - self.learning_rate * delta_w  # type: ignore

    def set_config(self, rpu_config: "DPRPUConfig") -> None:
...
class DPTile(TileModule, TileWithPeriphery, SimulatorTileWrapper):
    ...
    def __init__(
        ...
        out_size: int,
        in_size: int,
        rpu_config: Optional["DPRPUConfig"] = None,
        bias: bool = False,
        in_trans: bool = False,
        out_trans: bool = False,
    ):
    ...
    def _create_simulator_tile(  # type: ignore
        self, x_size: int, d_size: int, rpu_config: "DPRPUConfig"
    ) -> "SimulatorTile":
...
@dataclass
class DPUpdateParameters(_PrintableMixin):
...
@dataclass
class DPRPUConfig(MappableRPU, PrePostProcessingRPU):
    """Configuration for resistive processing unit using the CustomTile."""

    tile_class: ClassVar[Type] = DPTile
    """Tile class that correspond to this RPUConfig."""

    simulator_tile_class: ClassVar[Type] = DPSimulatorTile
    ...
    update: DPUpdateParameters = field(default_factory=DPUpdateParameters)
